...
public interface UserLinkedAccountRepository extends JpaRepository<UserLinkedAccount, String> {
    @Query(value = "SELECT CASE WHEN COUNT(id)> 0 THEN 'true' ELSE 'false' END FROM user_linked_account ula WHERE lower(ula.apps) LIKE lower(:apps) AND lower(ula.apps_customer_email) like lower(:email);", nativeQuery = true)
